...
async function sweepOnce(): Promise<void> {
  ...
  // shape as the success path: omit `media` on the event so the bubble
  // strips its media block and renders as a regular text row.
  for (const row of stuck) {
    emitToTeam(row.teamId, "message:media:ready", {
      teamId: row.teamId,
      conversationId: row.conversationId,
      messageId: row.id,
      // No media → client clears the placeholder.
    });
  }
