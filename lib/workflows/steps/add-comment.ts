...
export const addCommentStepHandler: StepHandler<AddCommentStepConfig> = {
  ...
  async run(envelope, config, ctx): Promise<StepResult> {
    ...
    // Mirror what the inbox internal-note POST does — emit on the conversation
    // room so live viewers see the note appear without a refetch.
    emitToTeam(ctx.teamId, "note:new", {
      teamId: ctx.teamId,
      conversationId,
      note: {
        id: note.id,
