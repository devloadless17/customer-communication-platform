...
export function useTeamChannelEvents(
  ...
  const markRead = useCoalescedAsync(async () => {
    try {
      await fetchWithSessionGuard(`/api/team-chat/channels/${channelId}/read`, {
        method: "POST",
      });
    } catch {
    ...
    async (commit: (run: () => void) => void = flushSync): Promise<number> => {
      ...
      try {
        const res = await fetchWithSessionGuard(
          `/api/team-chat/channels/${channelId}/messages?before=${encodeURIComponent(cursor)}`,
        );
        if (!res.ok) return 0;
  ...
  const loadNewer = useCallback(async (): Promise<number> => {
    ...
    try {
      const res = await fetchWithSessionGuard(
        `/api/team-chat/channels/${channelId}/messages?after=${encodeURIComponent(newerCursor)}`,
      );
      if (!res.ok) return 0;
      const page = (await res.json()) as {
        items: TeamChannelMessageDto[];
  ...
  const loadAround = useCallback(
    async (messageId: string): Promise<string | null> => {
      ...
      try {
        const res = await fetchWithSessionGuard(
          `/api/team-chat/channels/${channelId}/messages/around?messageId=${encodeURIComponent(messageId)}`,
        );
  ...
  const goToLive = useCallback(async (): Promise<void> => {
    const res = await fetchWithSessionGuard(`/api/team-chat/channels/${channelId}/messages`);
    if (!res.ok) return;
    const page = (await res.json()) as {
      items: TeamChannelMessageDto[];
      nextCursor: string | null;
  ...
  useEffect(() => {
    ...
    const runBackfill = () => {
      ...
      void fetchWithSessionGuard(
        `/api/team-chat/channels/${channelId}/messages?after=${encodeURIComponent(cursor)}`,
      )
  ...
  const retryOptimistic = useCallback(
    (clientTempId: string) => {
      ...
      const url = `/api/team-chat/channels/${channelId}/messages`;
