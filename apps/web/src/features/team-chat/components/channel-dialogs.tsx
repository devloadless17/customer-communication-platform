...
export function NewChannelDialog({
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      const res = await fetchWithSessionGuard("/api/team-chat/channels", {
        method: "POST",
...
export function EditChannelDialog({
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      const res = await fetchWithSessionGuard(`/api/team-chat/channels/${channel.id}`, {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          name: normalized,
...
export function useDeleteChannel() {
  ...
  const deleteChannel = async (channelId: string, fallbackHref: string) => {
    ...
    if (!ok) return;
    const res = await fetchWithSessionGuard(`/api/team-chat/channels/${channelId}`, {
      method: "DELETE",
    });
    if (res.ok) {
