...
function ChannelMessageImpl({
  ...
  const toggleReaction = async (emoji: string) => {
    ...
    try {
      const res = await fetchWithSessionGuard(
        `/api/team-chat/channels/${channelId}/messages/${message.id}/reactions`,
        {
          method: "POST",
  ...
  const togglePin = async () => {
    ...
    try {
      const res = await fetchWithSessionGuard(
        `/api/team-chat/channels/${channelId}/messages/${message.id}/pin`,
  ...
  const submitDelete = async () => {
    ...
    try {
      const res = await fetchWithSessionGuard(
        `/api/team-chat/channels/${channelId}/messages/${message.id}`,
        { method: "DELETE" },
      );
      if (!res.ok) {
