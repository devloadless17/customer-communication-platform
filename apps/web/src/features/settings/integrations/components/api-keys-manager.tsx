            ...
            {!fullAccess && (
              <div className="mt-3 grid grid-cols-1 gap-1.5 sm:grid-cols-2">
                {GRANULAR_SCOPES.map((scope) => (
                  <label key={scope} className="flex cursor-pointer items-center gap-2 text-[11px]">
                    <input
                      type="checkbox"
                ...
                {chosenScopes.length === 0 && (
                  <p className="col-span-full text-[11px] text-destructive">Pick at least one scope.</p>
                )}
              </div>
            )}
          </div>
              ...
              <ul className="divide-y divide-border rounded-md border border-border">
                {keys.map((k) => (
                  <li key={k.id} className="flex flex-wrap items-center gap-x-3 gap-y-1 px-3 py-2.5">
                    ...
                    <code className="font-mono text-[11px] text-muted-foreground">{k.tokenPrefix}…</code>
                    ...
                    <div className="ml-auto flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        onClick={() => revoke(k)}
                        disabled={busyId === k.id}
                        className="inline-flex items-center gap-1 rounded-md border border-border bg-background px-1.5 py-0.5 text-[10px] font-medium text-destructive hover:bg-destructive/10 disabled:cursor-not-allowed disabled:opacity-50"
