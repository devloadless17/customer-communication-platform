...
export async function sendMediaInternal(
  ...
  // Store the mp3 in R2 (deterministic key). putObject skips the media
  // magic-byte sniff — these are first-party generated bytes.
  ...
  if (binding.provider.capabilities.mediaSendByUrl) {
    ...
    const up = await uploadMedia(
      { bytes: args.bytes, mimeType: args.mimeType, filename: args.filename },
      sendConfig,
    );
    mediaId = up.mediaId;
  }
