...
export async function runAiReply(job: AiReplyJob): Promise<void> {
  ...
  if (payload.complaintConfidence >= COMPLAINT_FLAG_THRESHOLD) {
    await flagComplaint(teamId, inboundMessageId, payload.complaintConfidence).catch(() => {});
  }
...
/**
 * Auto-raise the team's "Complaint" flag definition on the customer's inbound
 * message. Best-effort and silent on any failure (definition renamed/deleted/
 * archived, race, etc.) — a missed auto-flag is a triage inconvenience, never
 * worth failing the reply over. `raiseFlag` is idempotent (unique on
 * messageId+definitionId), so an at-least-once redelivery can't double-raise.
 */
async function flagComplaint(teamId: string, messageId: string, confidence: number): Promise<void> {
  const definition = await db.messageFlagDefinition.findFirst({
    where: { teamId, name: "Complaint", archived: false },
    select: { id: true },
  });
