...
export const auth = betterAuth({
  ...
  // existing sessions; it never issues or revokes cookies. Sign-in/out write
  // Set-Cookie via Better Auth's catch-all route on the Next.js side, which
  // has its own `apps/web/src/lib/auth/better-auth.ts` with the plugin.
  plugins: [],
});
