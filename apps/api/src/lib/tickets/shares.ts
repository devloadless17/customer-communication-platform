...
// ---------------------------------------------------------------------------
// Comments — the conversation BETWEEN the departments.
// ---------------------------------------------------------------------------
...
/**
 * Post a comment on the ticket. Everyone with access sees it — that IS the
 * cross-department conversation. (An internal `note` is the other tool: it
 * stays inside the workspace that wrote it.)
 *
 * Not a ticket UPDATE: a comment changes nothing about the work, so it must not
 * bump `version` (which would 409 a colleague's open editor) or move the SLA
 * clock. Returns the event id so attachments can be filed against it.
 */
export async function addTicketComment(
  ...
  const ticket = await db.ticket.findFirst({
    where: ticketByIdWhere(args.workspaceId, args.ticketId),
    select: { id: true, workspaceId: true, conversationId: true, shares: { select: { id: true } } },
  });
  if (!ticket) return { ok: false, reason: "ticket_not_found" };

  const eventId = await db.$transaction(async (tx) => {
    const id = await writeTicketEvent(
      tx,
      ticket.workspaceId,
      ticket.id,
      "escalation_note",
    ...
    // Only worth a realtime frame when someone else is actually watching this
    // ticket from another workspace; an unshared ticket's own view refetches
    // after the POST, exactly like an internal note.
    if (ticket.shares.length > 0) {
      const mapped = await readTicket(tx, ticket.id);
      const openTicketCount = ticket.conversationId
        ? await bumpOpenTicketCount(tx, ticket.conversationId, 0)
        : 0;
      await publishTicketEvent(tx, {
        // A comment is not a state change: it must never chain a workflow or
        // echo as a lifecycle webhook.
        args: {
