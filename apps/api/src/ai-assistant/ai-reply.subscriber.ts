...
export class AiReplySubscriber implements OnModuleInit, OnModuleDestroy {
  ...
  private async onMediaReady(e: DomainEventOf<"message.media_ready">): Promise<void> {
    ...
    if (kind === "audio") {
      // STT transcription only ever runs on OpenAI (see reply-text engine
      // comment in models.ts) — that stays a hard OpenAI requirement even
      // when reply TEXT is generated by Anthropic.
