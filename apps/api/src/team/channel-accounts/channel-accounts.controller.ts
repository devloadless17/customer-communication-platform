 ...
 *
 *   GET    /api/team/channels/:channel/accounts
 *   PATCH  /api/team/channels/:channel/accounts/:id        — rename
 *   POST   /api/team/channels/:channel/accounts/:id/default
 *   DELETE /api/team/channels/:channel/accounts/:id        — disconnect
 *
 * ADDING an account is not here: it's the existing per-channel connect flow
 * (POST /api/team/whatsapp etc.), which upserts keyed on the provider's account
 * id — so pasting a second number's credentials creates a second account rather
 * than overwriting the first.
 */
@Controller("api/team/channels/:channel/accounts")
@RequireRole("admin")
