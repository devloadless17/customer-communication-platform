...
test("channel rows and DM rows share the same inset — the active row never touches the rail", async ({
  ...
  await request.post("/api/team-chat/channels/dm", { data: { userId: peerUserId } });
...
test("a search-result highlight is dismissible and does not survive a reload", async ({
  ...
  const list = await (
    await request.get("/api/team-chat/channels?take=50")
  ).json();
  const channelId: string = (list.items ?? list.channels ?? list)[0].id;

  const msgs = await (
    await request.get(`/api/team-chat/channels/${channelId}/messages?take=50`)
  ).json();
...
test("the reaction endpoint refuses a non-emoji", async ({ page, request }) => {
  ...
  const bad = await request.post(
    `/api/team-chat/channels/${channelId}/messages/${hit.id}/reactions`,
...
test("a garbage ?take returns a 400, not a 500", async ({ page, request }) => {
  ...
  for (const path of [
    `/api/team-chat/channels/${channelId}/messages?take=abc`,
    `/api/team-chat/channels/${channelId}/messages/search?q=hello&take=abc`,
    `/api/team-chat/channels/search?q=hello&take=abc`,
  ]) {
  ...
  const neg = await request.get(
    `/api/team-chat/channels/${channelId}/messages?take=-1`,
  );
...
test("a DM with a departed teammate is read-only, in the UI AND on the server", async ({
  ...
  try {
    ...
    // could walk around.
    const res = await request.post(`/api/team-chat/channels/${dm.id}/messages`, {
...
test("a self-DM stays writable — it has no peer to deactivate", async ({ request }) => {
  const dm = (
    await (
      await request.post("/api/team-chat/channels/dm", { data: { userId: adminUserId } })
    ).json()
  ).channel;
  const res = await request.post(`/api/team-chat/channels/${dm.id}/messages`, {
