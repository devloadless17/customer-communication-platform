 ...
 * PATCH /api/admin/teams/:id/max-members). Enforced at invite-create (soft)
...
test.describe("org member limit", () => {
  test("superAdmin sets, validates, and reads back an org's member cap", async ({
    ...
    try {
      ...
      // Unknown team → 404.
      expect(
        (await su.patch(`/api/admin/teams/does-not-exist/max-members`, { data: { maxMembers: 3 } })).status(),
      ).toBe(404);
    } finally {
      await su
  ...
  test("a non-superAdmin cannot change a member cap", async ({ request }) => {
    ...
    const res = await request.patch("/api/admin/teams/any-id/max-members", {
      data: { maxMembers: 9 },
    });
