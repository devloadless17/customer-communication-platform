...
- `external/v1/external-v1.controller.ts` split by resource — **IN PROGRESS, safely resumable**: request guards extracted (`1844968b`); WORKFLOWS / CUSTOMERS / OUTBOUND WEBHOOKS / AUDIENCE GROUPS+SNIPPETS / TICKETS+escalation / CONVERSATION OPS / BROADCAST WRITES peeled (`e0535189`, `9e3ee429`, `51e52f74`, `d4f5c648`) — 3,540 → 2,428 lines, eight controllers. **check:v1-docs now DISCOVERS every *.controller.ts under external/v1** (the first peel silently dropped its routes from a hand-named-file count, 197 → 191 with a green tick — fixed + negative-tested), so any session can continue peeling: remaining inside the main controller: the interleaved Channel-accounts / Instagram / Messenger / WhatsApp-settings / flags / views block (~1,360 L — needs finer per-route mapping, not a section cut) and the contacts/messages/sends core. Operational note: tsx watch stops watching a file that never loaded — after fixing a boot error, `touch apps/api/src/main.ts` to revive the reload. Sealed by full api vitest (exit 0) + live 401 probes per peel.
