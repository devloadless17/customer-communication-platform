...
- ~~`external/v1/external-v1.controller.ts` split by resource~~ — **DONE post-program** (`1844968b` → `cc097829`): fifteen controllers — workflows, customers, outbound-webhooks, catalog, tickets+escalation, conversation-ops, broadcasts (reads+writes), assignment, flags, inbox-views, whatsapp, messenger, channels+instagram+calls, contact-transfers — around a 1,377-line contacts/messages/sends core that deliberately keeps the ExternalV1Controller name. Shared request guards in v1-request-guards.ts; **check:v1-docs now DISCOVERS every *.controller.ts** (the first peel proved the hand-named-file version silently drops routes — negative-tested). Sealed by full api vitest (exit 0), lint 0, live 401 probes per surface. Ops notes: tsx watch stops watching a file that never loaded (`touch main.ts` revives); never regex-prune import interiors (mangled two multi-line imports; caught by tsc, restored from git).
...
- `lib/providers/meta.ts` split — **stage 1 DONE** (`21c3f397`): 822 lines of pure template parse/build → `meta-template-parse.ts` behind the file's established re-export facade; transport wrappers deliberately stayed. 8,144 → 7,364. **Stage 2 DONE** (`HEAD`): the webhook-parse family (19 parsers + 12 wire interfaces, 1,882 lines) → `meta-webhook-parse.ts`; meta.ts at 5,519; meta e2e 170/170 incl. @pressure. **Stage 3 DONE**: meta-transport.ts (metaFetch core, 208 L) + meta-send.ts (senders, 472 L), layered meta → send → transport with the parse modules as leaves; meta.ts at 4,900 (was 8,144); meta e2e 170/170 + full vitest. The meta.ts split is COMPLETE.
