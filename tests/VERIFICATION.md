...
| Finding | Where | Notes |
|---|---|---|
| **Every deploy shows a hard error page for a few seconds.** Compose (`/opt/ccp`, service `app` = container `ccp-app-1`; there is no service named `web`) recreates api first and `app` last. The api container runs `prisma migrate deploy` as the first step of its CMD, so it sits at `health: starting` for as long as the migration stack takes (~60-120s on a batch like this release's six). `app` comes up inside that window, and its RSC pages fetch the API over `INTERNAL_API_URL` **during render** — so the render throws and the user gets "Something broke." with a digest instead of a retry. Observed live on the 2026-07-27 deploy (run 30279058678 — all 8 gates green; this window is invisible to the smoke check, which runs after health). Fix shape: retry/backoff around the RSC session+data fetch, or hold the request at Caddy until the API healthcheck passes. Not a regression from any one change — it is how the deploy has always behaved. | `apps/web/src/lib/auth/current-user.ts` + every RSC page fetch; the `app` service's `depends_on` in docker-compose; Caddyfile | Deliberately NOT hot-fixed under deploy pressure — belongs in the rubric pass with the loading/error-state row it falls under |
