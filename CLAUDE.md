...
- **Socket.io connection-state-recovery is bounded at 2 minutes** ([apps/api/src/realtime/ws-adapter.ts](apps/api/src/realtime/ws-adapter.ts)). After a longer offline (laptop sleep, WiFi hop), non-displayed cached threads in the LRU are evicted on the next reconnect; clicking back to one triggers a refetch. This is the documented fallback, not a bug. Don't extend the recovery window or build a custom replay layer — the eviction-then-refetch path is the simple correct answer.
