...
1. **No process supervisor — `docker compose` drives the stack directly** (NOT pm2, NOT systemd). The `ccp` systemd unit was REMOVED 2026-05-26: it ran `docker compose up` with `Restart=always`, which fought every manual `docker stop`/`down` (the unit's `up` recreated whatever you stopped) and never pruned old images, so the VPS disk filled. The deploy job now SSHes in and runs `docker compose --env-file .env up -d --remove-orphans` + `docker image prune -f` directly, and auto-removes the legacy unit if found. **Restart policy is `restart: "no"` on every prod service** — fully manual, nothing auto-starts (a `down`/`stop` stays down; a VPS reboot leaves the app down until you `docker compose up -d`). Graceful-shutdown drain is now bounded by compose's **`stop_grace_period`** (100s on api, 30s on web — see [docker-compose.yml](docker-compose.yml)), NOT systemd `TimeoutStopSec`; the 100s on api still exceeds the 90s BullMQ `lockDuration` plus tail. The Dockerfile's `ENV NODE_OPTIONS=--max-old-space-size=4096` carries the heap (env-on-host never propagated through compose anyway). Re-enabling auto-start-on-boot WITHOUT the old bounce-back bug = set `restart: unless-stopped` in compose (Docker's own policy honors manual stop), never a systemd `up` wrapper. See "Graceful shutdown" below and [deploy/README.md](deploy/README.md) "Starting / stopping the stack".
