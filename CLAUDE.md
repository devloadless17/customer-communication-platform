...
- **Workspace vs Team — the rule.** A **workspace** is HARD separation (its own
  ...
  — the single definition of "mine OR shared with me". The departments TALK in
  `TicketMessage` (the ticket **thread**, its own entity so the conversation
  isn't buried in the audit log — see §18), while `TicketEvent.actorWorkspaceId`
  makes the one shared HISTORY readable ("Billing changed the status"), and
  `TicketAttachment` files are visible to every party. An internal **note** is the
  other tool and is PRIVATE to the workspace that wrote it (`noteVisibilityWhere()`
  — it is stored with the owner's `workspaceId`, so nothing enforced that until
  2026-07-31). (This superseded a twin-pair
...
A conversation is the long-lived thread. A **ticket** is one piece of *work* on it, and there are many over time — the refund in March and the delivery question in June are two tickets on one unbroken thread, each with its own assignee, priority, SLA clock and outcome. Tickets are **raised deliberately** — by the agent who read the message (or a workflow's `create_ticket` step), with a subject, a priority and an assignee. There is **no auto-open** (the `Workspace.ticketAutoOpen` toggle was removed 2026-07-25): the inbox already tracks every thread, so auto-opening would make a ticket mean the same thing as a conversation and fill the board with work nobody raised. Ingest only ATTACHES an inbound to the thread's active ticket — otherwise the message carries no ticket at all. **Nothing opens or reopens a ticket but a person raising one.** Auto-REOPEN was removed 2026-08-01 (`Workspace.ticketReopenWindowHours` is retired and read by nothing): a customer's follow-up used to drag a solved ticket back to `open` with a "System reopened #10" line nobody asked for. Solved means the customer got their answer and the work is done, so a later message is small talk or a NEW issue — and a new issue deserves its own cause and its own number. A person may still reopen one deliberately. The inbox is untouched either way (the board is a parallel lens joined by `Message.ticketId`). See [`apps/api/src/lib/tickets/mutations.ts`](apps/api/src/lib/tickets/mutations.ts).
...
**Current state**: **shipped** (built alongside Messenger + Instagram). The `Customer` model + `Contact.customerId` exist; auto-merge on exact phone/email runs via `IdentityService` (inline at ingest + drift sweeper); manual reversible merge/split (`CustomersService.link`/`unlink`) + the "linked channels" switcher UI are live. Per-`Customer` **omnichannel broadcast targeting was built and then REMOVED (2026-07-27)** — `targetMode` is pinned to `'contact'` (`broadcasts.schemas.ts`) and the runner refuses legacy `'customer'` rows loudly ("recreate as one broadcast per channel"); `bestChannelForCustomer` lives on in the WORKFLOW `customer`/`trigger_customer` step targets, which are the omnichannel per-person reach today. **Not yet built**: lifting person-level fields onto `Customer` + a contacts-list page rollup by person. Gaps tracked in [`apps/api/src/lib/identity/identity-service.ts`](apps/api/src/lib/identity/identity-service.ts).
...
- **Frontend**: pure reducers (`apps/web/src/features/inbox/lib/thread-reducers.ts`) with three consumers and a dev-time `assertReducerCoverage` invariant plus a monotonic message-status guard. The live hook and the LRU cache shell iterate `THREAD_REDUCER_EVENTS` (table-driven — a new entry auto-wires both); the **contact panel deliberately subscribes directly** and derives scalar mirrors from covered events, because it renders from the cacheTick-silent LRU snapshot and routing it through `thread.data` would re-render the whole shell per inbound message — the reducers-file header documents this and forbids "fixing" it, and `apps/web/test/contact-panel-coverage.spec.ts` pins that every event it binds stays declared. Wire a new per-thread event into the TABLE (or exclusions, with a reason) or a chat-switch reverts to a stale snapshot.
...
- **A ticket's CAUSE is written once.** Fillable while empty, then immutable (`cause_immutable`) — clearing counts as rewriting. Everything after it (thread messages, notes, status moves, files) reasons against it, so a rewrite silently changes what the whole history was about. Updates travel in the thread. — [`apps/api/src/lib/tickets/mutations.ts`](apps/api/src/lib/tickets/mutations.ts)
- **A notification belongs to a PERSON, and never to the person who caused it.** `Notification` rows are per-user and append-only — one per event, **grouped by ticket when the bell RENDERS**, because coalescing at write time needs a read-then-write on "the unread row for this (user, ticket, actor)", which wants a partial unique index, and raw partial indexes are invisible to `migrate diff` and `check:prisma-fields` (see the raw-index invariant below). `notifyUsers()` filters the actor out ONCE, at the single write site: self-notification is what makes a bell something people mute. It is written AFTER the transaction and swallows its own failure — a courtesy attached to a real write must never fail that write. Whoever RAISED the ticket (`Ticket.createdById`) is in the audience for every change, alongside each side's assignee; deliberately NOT every member of a participating workspace. There is **no `/v1` surface**: a notification belongs to a person and an API key has no person, the same reasoning as the thread's read route. — [`apps/api/src/lib/notifications/notifications.ts`](apps/api/src/lib/notifications/notifications.ts)
- **The board sorts by `Ticket.lastActivityAt`, and the keyset cursor sorts with it.** Not `updatedAt` (that means "this row was written" — a reply deliberately doesn't write it, an SLA sweep does) and not a relation `max()` (Prisma can't express it in `orderBy`, and raw SQL here would hand-write the one query `ticketAccessWhere()` composes into). A cursor keyed on a different column than the ORDER BY silently skips and repeats rows, so `nextCursor` is `{ activityAt, id }`. Recomputable ⇒ reconciled by `ticket-last-activity-drift` (§7), which is load-bearing rather than ceremonial: `touchTicketActivity` swallows its own failure by design. — [`apps/api/src/lib/tickets/queries.ts`](apps/api/src/lib/tickets/queries.ts)
- **The ticket THREAD is not the audit log, and a reply is not a change.** The log answers "who changed what"; the thread (`TicketMessage`) answers "what did Billing say", and mixing them buried the one answer people came for among twenty status flips. So: a thread message publishes ONLY `ticket.thread_message_created` — never `ticket.changed`, which would re-broadcast the whole Ticket to every participating workspace and make every board, sub-sidebar and rail there re-run its counts, on the highest-frequency write a ticket has. It bumps no `version` (a reply must not 409 a colleague's open editor) and moves no SLA clock. Unread is a per-user MARKER row (`TicketThreadUnread`), not a `lastReadAt` watermark: a watermark needs a column-to-column comparison Prisma can't express, and the raw SQL would be a second hand-written copy of `ticketAccessWhere()`. It is counted by ANDing `unreadRepliesWhere()` into the same scope the board already uses, so revoking a share drops the badge with no second copy of the tenancy rule — and it is cleared only when the tab is genuinely visible (§10). — [`apps/api/src/lib/tickets/thread.ts`](apps/api/src/lib/tickets/thread.ts)
...
- **The AI assistant never asks a customer for a PHONE NUMBER**, and `phone` is not in the collectible-target vocabulary that the settings picker and the save-time validator both read (`COLLECT_BUILTIN_TARGETS`). `Contact.phoneNumber` is an *unconditional* auto-merge strong key (`findExistingCustomerIdByStrongKey`) while a number the customer TYPES is not vendor-verified — so a transposed digit or a partner's number fuses two people into one `Customer`, exposing one person's threads under the other's profile and misrouting a workflow's per-person (`customer` / `trigger_customer`) send. §6 permits a strong key only on a vendor-verified identity. Every target that IS offered is safe by construction: name is never a key (no fuzzy matching, ever), location is free text, and email is a key only under `trustEmailAsStrongKey`, which this path never sets. Collected values are re-validated server-side and only ever fill a BLANK — a model asked for a field will invent a plausible one. — [`packages/shared/src/ai/collect-details.ts`](packages/shared/src/ai/collect-details.ts), [`apps/api/src/lib/ai/contact-details.ts`](apps/api/src/lib/ai/contact-details.ts)
...
- **The ACTIVE WORKSPACE is resolved in exactly one place** — `resolveActiveWorkspaceId` (`@ccp/shared/auth/active-workspace`), called by the NestJS guard, the Socket.io handshake AND the Next.js RSC session. Order: membership-validated `ccp.ws` cookie → `Session.activeWorkspaceId` → first membership; the beyond-membership escape is always DB-verified, and its **two branches are separate on purpose**: an org owner/admin is **org-scoped** (that branch is the actual past vulnerability — a customer's owner reaching a sibling tenant — and never widens), while a platform superAdmin reaches **any workspace on the box** (OPERATOR MODE, below). Re-merging them is the bug. Three copies drifted once and the web silently rendered every switched session against the wrong workspace. Anything workspace-scoped that is cached (the session snapshot) is keyed by **(userId, workspaceId)**, and the per-user socket room is `user:<ws>:<uid>`. — §7
- **OPERATOR MODE: the platform operator administers every tenant, and is workforce in none.** A `User.isSuperAdmin` may enter ANY workspace with effective role `admin` while holding **no `WorkspaceMember` row** — that absence is the whole mechanism, because membership (`workspaceMemberships: { some: { workspaceId } }`) is what every people-surface filters on, so an administrator is structurally never counted as staff: no assignment pool, no availability schedule, no seat, no roster, no mention picker. Entry is an ACTION (`POST /api/admin/operator-access`), which writes an append-only `OperatorAccess` row and only then re-scopes the session — the record is the accountability, so it precedes the access rather than trailing it, and the platform org page renders the log beside the button. **The log is not a gate**: a superAdmin who hand-sets `ccp.ws` gets in without a row, accepted only because there is exactly one operator and they are the person the log describes — a MULTI-operator platform must make this branch consult an active grant instead of the flag. **Passive viewing writes nothing a tenant can observe**: `isOperator` (a superAdmin acting where they hold no membership — `isOperatorAccess`, computed identically by the HTTP guard, the socket handshake and the RSC session) suppresses mark-read (team unread is team-wide with no per-agent state, and it carries the Meta read receipt — blue ticks to the customer from a stranger), presence, viewer registration, and typing both team-facing and relayed to the customer. Not-registering beats filtering at emit time: `buildVisibleViewers` is fail-soft and returns the raw set on a DB blip. Actions the operator TAKES stay ordinary audited writes, with four hardening rules (2026-08-20 audit): **the SERVER is the only identity authority** — every DTO that joins an actor's name masks through `operator-mask.ts` (`actorNameMasker` batched, `maskMessageAuthors`/`maskFlagActors` post-passes; `scripts/check-actor-names.mjs` fails CI on an unmasked actor-name join), the client's roster-miss fallback means **"Former member"**, never the operator — an ex-member's User row survives a membership revoke, and only the server can tell the two apart; **an operator send never claims, marks read, or counts as a response** (`onAgentSendSideEffects` gates all agent-send side effects on one live membership read; agent-attributed analytics apply "don't register, rather than filter", so headline totals equal the sum of the agent rows); **org-shaped routes refuse in operator mode** (`assertNotOperator` — `session.organizationId` is the operator's own anchor inside a tenant, and `remove()` would have destroyed the operator's OWN workspace from a page about the tenant's; team-chat channel create/DM/join refuse too, because they mint `TeamChannelMember` rows); and **high-blast-radius actions self-record** on the `OperatorAccess` log (`recordOperatorAction`, awaited BEFORE the irreversible step: broadcast_send, api_key_create, outbound_webhook_create, contact_export). — [`packages/shared/src/auth/active-workspace.ts`](packages/shared/src/auth/active-workspace.ts), [`apps/api/src/lib/workspaces/operator-mask.ts`](apps/api/src/lib/workspaces/operator-mask.ts), [`apps/api/src/admin/admin-operator-access.controller.ts`](apps/api/src/admin/admin-operator-access.controller.ts)
