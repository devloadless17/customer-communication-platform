...
cp .env.example .env                # fill in BETTER_AUTH_SECRET, ENCRYPTION_KEY, INTERNAL_BUS_SECRET, UPLOADTHING_TOKEN
                                    # see "Secrets" below for what each one means
...
# in another shell:
pnpm run api:dev                    # NestJS on :4000
```
...
| Script | What it does |
|---|---|
| `pnpm run dev` | Next.js dev (`apps/web`) on :3000 |
| `pnpm run api:dev` | NestJS dev (`apps/api`) on :4000, swc-node loader |
...
- Run `pnpm run typecheck` AND smoke-boot both processes (`pnpm run dev` + `pnpm run api:dev`) before requesting review. Typecheck-green is necessary, not sufficient — NestJS DI failures and shared-lib regressions only surface at boot.
...
| `UPLOADTHING_TOKEN` | Sign up for your own UploadThing dev app | Free tier is enough. Don't share the production token. |
...
Caddy routing (prod): `/`, `/_next/*`, `/api/auth/*` → web. `/api/*`, `/socket.io/*`, `/webhooks/*` → api. The only exception: `/api/auth/change-password` lives on api despite the `/api/auth/` prefix. See [deploy/Caddyfile.example](deploy/Caddyfile.example).
...
- **Dev OOMs after a long edit session** (`FATAL ERROR: Ineffective mark-compacts near heap limit`) are a `tsx watch` + Next.js dev-mode artifact, not an app leak. Fix: `rm -rf .next` and restart `pnpm run dev`. Bump heap to 6GB if 4GB isn't enough during a heavy session.
...
- **Deploy artifacts:** [deploy/](deploy/) — Caddyfile example, systemd unit.
