...
/**
 * Read-only colored pill for the contacts list — shows the stage at a
 * glance without the management UI. When `clickable` is set, the chip
 * becomes a button whose onClick toggles a parent-managed picker.
 */
    ...
    if (onClick) {
      return (
        <button
          type="button"
          onClick={onClick}
          className={cn(dimClasses, "cursor-pointer hover:bg-accent hover:text-foreground")}
        >
          No stage
        </button>
      );
    }
    return <span className={dimClasses}>No stage</span>;
  }

  const colors = tagColorClasses(stage.color);
  const content = (
    <>
      <span className={cn("inline-block shrink-0 rounded-full", dotSize, colors.solid)} />
  ...
  if (onClick) {
    return (
      <button
        type="button"
        onClick={onClick}
        className={cn(sharedClasses, colors.chip, "cursor-pointer hover:brightness-110")}
      >
        {content}
      </button>
    );
  }
  return <span className={cn(sharedClasses, colors.chip)}>{content}</span>;
}
