...
- `work-hours` — **60s**. The only thing that notices a working-hours boundary: re-resolves every scheduled member's effective availability, moves them across shift start/end, and expires manual overrides. Writes and emits a socket frame ONLY on a real transition, and skips teams with no schedule entirely (so it costs nothing until an org configures hours). Being a full re-resolve each pass, it is also the drift sweeper that makes storing the effective status on `User.availabilityStatus` safe — a missed tick or a restart self-heals within one cadence.
- `message-flag-count-drift` — reconciles `Conversation.openFlagCount` against the actual unresolved `MessageFlag` rows. The counter is maintained transactionally by the flag mutations, so drift needs a crash between the flag write and the counter bump; this is the backstop that makes the denormalization safe to read on the hot list query.
