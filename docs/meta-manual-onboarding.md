...
| Verify token / callback URL | Pre-minted on our Messenger settings page → paste into App → **Messenger → Webhooks**, subscribe **`messages`**, then **subscribe the Page** (`POST /{pageId}/subscribed_apps?subscribed_fields=messages,messaging_postbacks`) |
...
| No inbound at all for a channel | The **Page isn't subscribed** to the app | `POST /{pageId}/subscribed_apps?subscribed_fields=messages,messaging_postbacks&access_token=PAGE_TOKEN` (or subscribe in the Meta UI). Verify with `GET /{pageId}/subscribed_apps`. |
